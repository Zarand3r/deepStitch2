...
class TransferLearning(LightningModule):
    ...
    def configure_optimizers(self):
            self.layers_to_fit = [{'params': self.feature_extractor.fc.parameters()}, {'params': self.feature_extractor.rnn.parameters()}]
            if self.feature_extractor.fc_pre_rgb != None:
                    self.feature_extractor.layers_to_fit.append({'params': self.feature_extractor.fc_pre_rgb.parameters()})
                    self.feature_extractor.layers_to_fit.append({'params': self.feature_extractor.fc_pre_of.parameters()})
            if self.feature_extractor.trainable_base:
                    self.feature_extractor.layers_to_fit.append({'params': self.feature_extractor.features_rgb.parameters()})
                    self.feature_extractor.layers_to_fit.append({'params': self.feature_extractor.features_of.parameters()})
            
            optimizer = torch.optim.Adam(self.feature_extractor.layers_to_fit,
                                                            lr=self.feature_extractor.hparams.lr, betas=(0.9, 0.999), 
                                                            weight_decay = self.feature_extractor.hparams.weight_decay)
