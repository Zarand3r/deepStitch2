...
class FusionModel(LightningModule):
    def __init__(self, args):
        ...
        if args.arch.startswith('alexnet'):
            ...
            self.fc_pre_rgb = nn.Sequential(nn.Linear(512 * 7 * 7, int(args.fc_size / 2)),
                                            nn.Dropout()) if 'conv' not in args.rnn_model else None
            self.fc_pre_of = nn.Sequential(nn.Linear(512 * 7 * 7, int(args.fc_size / 2)),
                                           nn.Dropout()) if 'conv' not in args.rnn_model else None
            self.final_channels = 512
        ...
        if args.rnn_model == 'LSTM':
            ...
            self.fc_k = nn.Linear(int(self.hparams.hidden_size) * nF * nF,
                                  self.num_classes * 10)  # replace with the dimensions of the kinematics data
            ...
            self.fc_attn1 = nn.Linear((int(self.hparams.hidden_size) + (2 * self.final_channels)) * nF * nF,
                                      ((int(self.hparams.hidden_size) + (
                                              2 * self.final_channels)) * nF * nF) // 2)
    ...
    def validation_end(self, outputs):
        ...
        top1_train = self.send_im_calculate_top1(self.actual_train, self.predicted_train, cmap_use='Oranges',
                                                 name='train/conf_mat')
        #####################################################################################
        self.actual = [];
        self.actual_train = []
        ...
        tensorboard_logs = {'val/loss': avg_loss, 'val/top1': top1_val, 'val/auc': auc_val, 'train/top1': top1_train,
                            'train/auc': auc_train, 'step': self.current_epoch}
    ...
    def train_dataloader(self):
        train_dataset = CustomDataset(self.hparams.datadir, idxs=self.hparams.idx_train,
    ...
    def val_dataloader(self):
        val_dataset = CustomDataset(self.hparams.datadir, idxs=self.hparams.idx_test,
                                    include_classes=self.hparams.include_classes,
    ...
    def apply_transforms_GPU(self, batch, random_crop=False):
        ...
        if random_crop:
            rgb = self.augGPU_resize(batch[0][:, :, :, :int(nW / 2), :].type(torch.float) / 255.,
                                     npix_resize=(240, 240), random_crop=True)
            of = self.augGPU_resize(batch[0][:, :, :, int(nW / 2):, :].type(torch.float) / 255., npix_resize=(240, 240),
                                    random_crop=True)
        else:
            rgb = self.augGPU_resize(batch[0][:, :, :, :int(nW / 2), :].type(torch.float) / 255.,
                                     npix_resize=(224, 224))
    ...
    def save_checkpoint(self, model, file_name="checkpoint.pth", is_best=0):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--include_classes', default='positive_negative', type=str,
                        help='Which classnames to include seperated by _ e.g. 00_01')
    ...
    #     'optimizer_state_dict': model.configure_optimizers()[0],
    #     'loss': model.loss,
    # }, PATH)
