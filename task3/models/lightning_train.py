...
class CustomDataset(Dataset):
    ...
    def __init__(self, global_dir, idxs=None, include_classes=[], flow_method='flownet', balance_classes=False, mode='train', max_frames=150, stride=2, masked="", upsample=False):
        ...
        if len(include_classes) == 0:
            ...
            self.remove_empty()
            fns = glob.glob(os.path.join(global_dir, '*',
                                         "optical_flow"+masked, '%s*' % flow_method))
        else:
            ...
            for class_curr in include_classes:
                fns.extend(glob.glob(os.path.join(
                    global_dir, class_curr, '%s*' % flow_method)))
            if len(fns) == 0:
                for class_curr in include_classes:
                    fns.extend(glob.glob(os.path.join(
        ...
        if len(fns) == 0:
            raise ValueError(
                'Likely that you have not pre-computed the optical flow or data directory is wrong!')
        ...
        if balance_classes:
            ...
            if upsample:
                ...
                for class_curr in class_counter.keys():
                    ...
                    self.filtered_fns.extend(random.choices(
                        [f for f in self.filtered_fns if f[1] == int(class_curr)], k=max(n_match-cnt, 1)))
            else:
                n_match = class_counter.most_common()[-1][1]
                for class_curr in class_counter.keys():
                    cnt = class_counter[class_curr]
                    print(class_curr, cnt)
                    indices = [index for index, x in enumerate(
                        self.filtered_fns) if x[1] == int(class_curr)]
...
class FusionModel(LightningModule):
    def __init__(self, args):
        ...
        for class_curr in self.hparams.include_classes:
            fns.extend(glob.glob(os.path.join(self.hparams.datadir,
                                              class_curr, '%s*' % self.hparams.flow_method)))
        if len(fns) == 0:
            for class_curr in self.hparams.include_classes:
        ...
        self.hparams.idx_train = idx[:int(
            self.hparams.train_proportion*len(idx))].copy()  # Save as hyperparams
        # Save as hyperparams
        self.hparams.idx_test = idx[int(
            self.hparams.train_proportion*len(idx)):].copy()
        ############################################################################################

        # Model specific
        original_model_rgb = models.__dict__[args.arch](
            pretrained=self.hparams.use_pretrained)
        original_model_of = models.__dict__[args.arch](
            pretrained=self.hparams.use_pretrained)
        ...
        if args.arch.startswith('alexnet'):
            ...
            self.fc_pre_rgb = nn.Sequential(nn.Linear(
                256*6*6, int(args.fc_size/2)), nn.Dropout()) if 'conv' not in args.rnn_model else None
            self.fc_pre_of = nn.Sequential(nn.Linear(
                256*6*6, int(args.fc_size/2)), nn.Dropout()) if 'conv' not in args.rnn_model else None
            ...
            self.features_rgb = nn.Sequential(
                *list(original_model_rgb.children())[:-2])
            for i, param in enumerate(self.features_rgb.parameters()):
                param.requires_grad = self.trainable_base
            self.features_of = nn.Sequential(
                *list(original_model_of.children())[:-2])
            for i, param in enumerate(self.features_of.parameters()):
                param.requires_grad = self.trainable_base

            self.fc_pre_rgb = nn.Sequential(nn.Linear(
                512*7*7, int(args.fc_size/2)), nn.Dropout()) if 'conv' not in args.rnn_model else None
            self.fc_pre_of = nn.Sequential(nn.Linear(
                512*7*7, int(args.fc_size/2)), nn.Dropout()) if 'conv' not in args.rnn_model else None
            self.final_channels = 512
            ...
            raise ValueError(
                'architecture base model not yet implemented choices: alexnet, vgg16, ResNet 18/34')
        # Select an RNN
        # print(self.features)
        if args.rnn_model == 'LSTM':
            ...
            self.rnn = ConvLSTMCell(input_channels=self.final_channels*2, hidden_channels=int(
                self.hparams.hidden_size), kernel_size=3, bias=True)

            nF = 6 if args.arch.startswith('alexnet') else 7
            # replace self.final_channels here the parameter must equal the hidden_channels in self.rnn
            self.fc = nn.Linear(int(self.hparams.hidden_size)
                                * nF*nF, self.num_classes)
        elif args.rnn_model == 'convttLSTM':
            # Twice number of channels for RGB and OF which are concat
            self.rnn = ConvTTLSTMCell(input_channels=self.final_channels*2,
                                      hidden_channels=self.final_channels, order=3, steps=5, ranks=16, kernel_size=3, bias=True)

            nF = 6 if args.arch.startswith('alexnet') else 7
            self.fc = nn.Linear(self.final_channels*nF*nF, self.num_classes)
        else:
            raise ValueError(
                'Not implemented, choose LSTM, convLSTM, convttLSTM type')

        self.modelName = '%s_%s_latefusion_trainbaseparams_%s' % (
            args.arch, args.rnn_model, args.trainable_base)
    ...
    def forward(self, inputs, hidden=None, steps=0):
        ...
        if 'conv' not in self.hparams.rnn_model:
            ...
            for kk in range(nFrames):
                f_all = []
                f = self.features_rgb(inputs[:, kk, :, :, :, 0].permute(
                    0, 3, 1, 2))  # permute to nB x nC x H x W
                f = f.reshape(f.size(0), -1)
                f = self.fc_pre_rgb(f)
                f_all.append(f)

                f_of = self.features_of(inputs[:, kk, :, :, :, 1].permute(
                    0, 3, 1, 2))  # permute to nB x nC x H x W
            ...
            for kk in range(nFrames):
                ...
                if kk == 0:
                    outputs = self.rnn(
                        torch.cat([f, f_of], dim=1), first_step=True)
                else:
                    outputs = self.rnn(
                        torch.cat([f, f_of], dim=1), first_step=False)
    ...
    def training_step(self, batch, batch_idx):
        ...
        input_cuda, target_cuda = self.apply_transforms_GPU(
            batch, random_crop=self.hparams.random_crop)
        ...
        self.predicted_softmax_train.append(
            softmax(output.detach().cpu().numpy(), axis=-1))  # Save scores
        tensorboard_logs = {'train/loss': loss}
        return {'loss': loss, 'log': tensorboard_logs}

    def validation_step(self, batch, batch_idx):
        input_cuda, target_cuda = self.apply_transforms_GPU(
            batch, random_crop=False)
        output, _, _ = self(input_cuda)
        output = output[:, -1, :]
        loss = F.cross_entropy(output, target_cuda.type(torch.long))
        ...
        if self.actual[-1] != self.predicted[-1]:
            # print self.filenames[batch_idx][0]
            print("MISCLASSIFIED: ", batch_idx)
        self.predicted_softmax.append(
            softmax(output.detach().cpu().numpy(), axis=-1))  # Save scores
    ...
    def validation_end(self, outputs):
        ...
        if self.trainer.overfit_pct == 0:  # Not a debug loop, otherwise roc_auc can throw errors
            if self.num_classes == 2:
                auc_train = roc_auc_score(self.actual_train, np.vstack(
                    self.predicted_softmax_train)[:, 1])
                auc_val = roc_auc_score(
                    self.actual, np.vstack(self.predicted_softmax)[:, 1])
            else:
                auc_train = roc_auc_score(self.actual_train, np.vstack(
                    self.predicted_softmax_train), multi_class='ovr')
                auc_val = roc_auc_score(self.actual, np.vstack(
                    self.predicted_softmax), multi_class='ovr')
        else:
            auc_train, auc_val = 0, 0

        top1_val = self.send_im_calculate_top1(
            self.actual, self.predicted, cmap_use='Blues', name='val/conf_mat')
        top1_train = self.send_im_calculate_top1(
            self.actual_train, self.predicted_train, cmap_use='Oranges', name='train/conf_mat')
        #####################################################################################
        self.actual = []
        self.actual_train = []
        self.predicted = []
        ...
        tensorboard_logs = {'val/loss': avg_loss, 'val/top1': top1_val, 'val/auc': auc_val,
                            'train/top1': top1_train, 'train/auc': auc_train, 'step': self.current_epoch}
    ...
    def send_im_calculate_top1(self, actual, predicted, cmap_use='Blues', name='tmp/name'):
        ...
        fig = plt.figure()
        sns.heatmap(cm, cmap=cmap_use, ax=plt.gca(), annot=True,
                    xticklabels=self.hparams.include_classes, yticklabels=self.hparams.include_classes)
        self.logger.experiment.add_figure(
            name, fig, global_step=self.current_epoch, close=True)
        top1 = float(
            sum([a == b for a, b in zip(actual, predicted)]))/len(actual)
        return top1

    def configure_optimizers(self):
        self.layers_to_fit = [{'params': self.fc.parameters()}, {
        ...
        if self.trainable_base:
            self.layers_to_fit.append(
                {'params': self.features_rgb.parameters()})
            self.layers_to_fit.append(
                {'params': self.features_of.parameters()})
    ...
    def train_dataloader(self):
        ...
        train_dataloader = DataLoader(train_dataset, batch_size=self.batch_size,
                                      shuffle=True, num_workers=self.hparams.number_workers, drop_last=True)
    ...
    def val_dataloader(self):
        ...
        val_dataloader = DataLoader(val_dataset, batch_size=self.batch_size,
                                    shuffle=False, num_workers=self.hparams.number_workers, drop_last=True)
    ...
    def apply_transforms_GPU(self, batch, random_crop=False):
        ...
        if random_crop:
            rgb = self.augGPU_resize(batch[0][:, :, :, :int(
                nW/2), :].type(torch.float)/255., npix_resize=(240, 240), random_crop=True)
            of = self.augGPU_resize(batch[0][:, :, :, int(
                nW/2):, :].type(torch.float)/255., npix_resize=(240, 240), random_crop=True)
        else:
            rgb = self.augGPU_resize(batch[0][:, :, :, :int(
                nW/2), :].type(torch.float)/255., npix_resize=(224, 224))
            of = self.augGPU_resize(batch[0][:, :, :, int(
                nW/2):, :].type(torch.float)/255., npix_resize=(224, 224))
        ...
        rgb = self.augGPU_normalize_inplace(
            rgb, mean=[0.3, 0.2, 0.2], std=[0.2, 0.2, 0.2])
        of = self.augGPU_normalize_inplace(
            of, mean=[0.99, 0.99, 0.99], std=[0.005, 0.005, 0.005])
        #print(of.reshape(-1, 3).mean(0))
    ...
    def augGPU_resize(self, input, seed=None, npix_resize=(224, 224), random_crop=False):
        ...
        resized_rgb = torch.zeros(
            nB, nF, npix_resize[0], npix_resize[1], nC).type_as(input)
        for bb in range(nB):
            for ff in range(nF):
                im = input.permute(0, 1, 4, 2, 3)[
                    bb, ff, :, :, :].unsqueeze(0).type(torch.float)
                resized_rgb[bb, ff, :, :, :] = F.interpolate(
                    im, size=npix_resize).permute(0, 2, 3, 1)
    ...
    def augGPU_normalize_inplace(self, input, mean=[0.3, 0.3, 0.3], std=[0.1, 0.1, 0.1]):
        ...
        input = input.sub_(mean[None, None, None, None, :]).div_(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--loadchk', default='',
                        help='Pass through to load training from a checkpoint')
    parser.add_argument(
        '--datadir', default=settings.label_directory, help='train directory')
    ...
    parser.add_argument('--trainable_base', default=0, type=int,
                        help='Whether to train the feature extractor')
    parser.add_argument('--rnn_model', default='LSTM',
                        type=str, help='RNN model at clasification')
    ...
    parser.add_argument('--fc_size', default=32, type=int,
                        help='size of fully connected layer before rnn')
    parser.add_argument('--epochs', default=60, type=int,
                        help='manual epoch number')
    parser.add_argument('--lr', default=0.0001, type=float,
                        help='initial learning rate')
    parser.add_argument('--lr_lambdas', default=0.9,
                        type=float, help='Schedulre hyperparam')
    parser.add_argument('--include_classes', default='positive_negative',
                        type=str, help='Which classnames to include seperated by _ e.g. 00_01')
    parser.add_argument('--flow_method', default='flownet',
                        type=str, help='Which flow method to use (flownet or dali)')
    parser.add_argument('--random_crop', default=0, type=int,
                        help='Whether or not to augment with random crops...')
    ...
    parser.add_argument('--loader_nframes', default=140,
                        type=int, help='How many frames to load at stride 2')
    parser.add_argument('--loader_stride', default=2,
                        type=int, help='stride for dataloader')
    parser.add_argument('--number_workers', default=0,
                        type=int, help='number of workers for Dataloader')
    parser.add_argument('--batch_size', default=1, type=int, help='batch size')
    parser.add_argument('--masked', dest='masked',
                        action='store_true', help="train on masked?")
    parser.add_argument('--upsample', dest='upsample',
                        action='store_true', help="upsample or downsample?")
    ...
    if hparams.include_classes == '':
        raise ValueError(
            'Please define the classes to use using the 00_01 underscore notation')
    else:
    ...
    print("==> creating model FUSION '{}' '{}'".format(
        hparams.arch, hparams.rnn_model))
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        filepath=os.path.join(settings.checkpoints3, classification_name,
                              f'{hparams.arch}_{hparams.trainable_base}_{hparams.rnn_model}{hparams.masked}'),
        save_top_k=3,
        verbose=True,
        monitor='val_acc',
        mode='max',
