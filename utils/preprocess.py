...
def get_kinematics(kinematics_directory, kinematicsfile, filename, start, end):
	...
	with open(os.path.join(kinematics_directory, "demonstrations", filename+".csv"), 'w') as csvfile: 
...
def main(labelsfile, data_directory, kinematics_directory):
	...
	for index, row in labels.iterrows():
		...
		video_output_fn = os.path.join(data_directory, "demonstrations", filename)
		...
		os.system(cmd)
		kinematics = get_kinematics(kinematics_directory, kinematicsfile, filename[:-4], start, end)
