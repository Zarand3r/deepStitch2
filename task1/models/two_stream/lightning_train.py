    ...
    def __init__(self, global_dir, idxs=None, include_classes=[], flow_method='flownet', balance_classes=False,
        ...
        if len(include_classes) == 0:
            ...
            for class_curr in include_classes:
                fns.extend(
                    glob.glob(os.path.join(global_dir, class_curr, "optical_flow" + masked, '%s*' % flow_method)))
        if len(fns) == 0:
        ...
        if balance_classes:
            ...
            for class_curr in class_counter.keys():
                ...
                self.filtered_fns.extend(
                    random.choices([f for f in self.filtered_fns if f[1] == int(class_curr)], k=max(n_match - cnt, 1)))
...
class FusionModel(LightningModule):
    def __init__(self, args):
        ...
        self.outputs = torch.zeros(1, 0, 6, 6)
        ############################################################################################
        # Generate the tra300in and test splits
        fns = []
        for class_curr in self.hparams.include_classes:
            fns.extend(glob.glob(os.path.join(self.hparams.datadir, class_curr, "optical_flow" + self.hparams.masked,
                                              '%s*' % self.hparams.flow_method)))
...
if __name__ == '__main__':
    ...
    checkpoint_callback = ModelCheckpoint(
        filepath=os.path.join(settings.checkpoints1, "two_stream", classification_name,
