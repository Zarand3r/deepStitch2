...
class FusionModel(LightningModule):
	...
	def train_dataloader(self):
		...
		train_dataloader 	= DataLoader(train_dataset, batch_size=self.batch_size, shuffle=True, num_workers=28)
	...
	def val_dataloader(self):
		...
		val_dataloader 	= DataLoader(val_dataset, batch_size=self.batch_size, shuffle=False, num_workers=28)
...
if __name__ == '__main__':
	...
	checkpoint_callback = ModelCheckpoint(
		...
		filepath=os.path.join(settings1.checkpoints, "one_stream", classification_name, f'{hparams.arch}_{hparams.trainable_base}_{hparams.rnn_model}'),
		save_top_k=3,
		verbose=True,
		monitor='val_acc',
		mode='max',
